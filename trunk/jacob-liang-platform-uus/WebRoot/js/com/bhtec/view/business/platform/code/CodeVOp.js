...
com.bhtec.view.business.platform.code.CodeVOp = function(config){
	...
	var funForm = function(configForm){
		...
		var codeEngName = moduleVOp.textField({
				...
				name : "codeEngName",
				allowBlank:false,
				fieldLabel : getFormFieldName(xmlDoc,'codeEngName'),
		...
		var codeName = moduleVOp.textField({
				name : "codeName",
				allowBlank:false,
				fieldLabel : getFormFieldName(xmlDoc,'codeName'),
		...
		var moduleName = moduleVOp.triggerField({
				id : 'moduleNameId',
				name : 'moduleName',
				fieldLabel : getFormFieldName(xmlDoc,'moduleName'),
		...
		var delimite = moduleVOp.comboBox({
				id : "delimite",
				fieldLabel : getFormFieldName(xmlDoc,'delimite'),
		...
		var codeEffect = moduleVOp.textField({
				...
				name:'codeEffect',
				readOnly:true,
				fieldLabel : getFormFieldName(xmlDoc,'codeEffect'),
		...
		var disableCodeFormat = function(partNumArr){
			...
			for(j=0,l=partNumArr.length;j<l;j++){
		...
		var partNum = moduleVOp.comboBox({
				id : "partNum",
				fieldLabel : getFormFieldName(xmlDoc,'partNum'),
				store:new Ext.data.JsonStore({
		...
		var formatDate = function(format){
			...
			if(format == 'yyyy'){
				...
				return d.getYear()+''+monthStr;
			}else if(format == 'yyyyMMdd'){
				d = new Date();
				month = d.getMonth()+1;
				var monthStr = month<10?'0'+month:month+'';
				return d.getYear()+''+monthStr +''+ d.getDate();
		...
		for(i=1;i<5;i++){
			...
			var codeFormatSelect = moduleVOp.comboBox({
					id : "codeFormat"+i,
					fieldLabel:i+getFormFieldName(xmlDoc,'part'),
					store:new Ext.data.JsonStore({
			...
			var codeFormatContent = moduleVOp.comboBox({
					...
					fieldLabel : i+getFormFieldName(xmlDoc,'partCon'),
					maxLength:20,
					store:new Ext.data.JsonStore({
						data : [],
