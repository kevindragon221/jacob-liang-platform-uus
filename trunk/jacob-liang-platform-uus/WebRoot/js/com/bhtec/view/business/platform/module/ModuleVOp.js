...
com.bhtec.view.business.platform.module.ModuleVOp = function(config){
	...
	var busiForm = function(configForm){
		...
		var saveCommon = function(saveFlag) {
			...
			var configSave = {
				url : saveFlag.url,
				params : {
					privilegeLevel : getExtCmpValueById('privilegeLevel'),
		...
		var modImgCls = moduleVOp.textField({
				...
				fieldLabel : getFormFieldName(xmlDoc,'modImgCls'),
				vtype:'alphanum'
		});
		...
		var modLevel = moduleVOp.comboBox({
				id : "modLevel",
				fieldLabel : getFormFieldName(xmlDoc,'modLevel'),
		...
		var modOrder = moduleVOp.comboBox({
				name : "modOrder",
				fieldLabel : getFormFieldName(xmlDoc,'modOrder'),
				store:new Ext.data.JsonStore({
		...
		//权限级别
		var privilegeLevel = moduleVOp.comboBox({
				id : "privilegeLevel",
				fieldLabel : getFormFieldName(xmlDoc,'privilegeLevel'),
				store:new Ext.data.JsonStore({
				...
				allowBlank:false,
				value:moduleData.privilegeLevel||'without'
		});
		...
		var upModule = moduleVOp.triggerField({
				id : 'upModNameId',
				name : 'upModNameId',
				fieldLabel : getFormFieldName(xmlDoc,'upModule'),
		...
		moduleVOp.cuvWindow({
			...
			columnFields:[modName,modEnId,modImgCls,
						  modLevel,modOrder,status,privilegeLevel,upModule,
	...
	var moduleLabelOp = function(){
		...
		//通过菜单控制获得参数
		var moduleLabel = new com.bhtec.control.MenuControl({
			id:modDelRecord.modEnId,
			moduleLabel:true
		});
		//模块标签列表
		var moduleLabelList = moduleLabel.moduleLabelList;
		var xmlFile = moduleLabel.xmlFile;//xml文件路径
		var colsArray = new Array();//动态构造列
		for(i=0;i<moduleLabelList.length;i++){
			var modLabel = moduleVOp.textField({
					id : moduleLabelList[i].fieldName,
					maxLength  : 30,
					allowBlank:false,
					fieldLabel : '模块标签'+(i+1),
					value:moduleLabelList[i].fieldLabel||''
			});	
		...
		moduleVOp.cuvWindow({
			...
			save:function(){							//保存按钮调用的方法
				var moduleLabelArray1 = new Array();
				for(i=0;i<moduleLabelList.length;i++){
					var moduleLabelArray2 = new Array();
					moduleLabelArray2.push(moduleLabelList[i].fieldName);
					moduleLabelArray2.push(getExtCmpValueById(moduleLabelList[i].fieldName));
					moduleLabelArray1.push(moduleLabelArray2)
				}
				ajaxRequest({
					url:'moduleAction!modifyModuleLabel.action',
					params:{
						xmlFile:xmlFile,
						moduleLabelListStr:Ext.encode(moduleLabelArray1)//二维数组
					},
					callBack:function(returnData){
						var configCb = {
							msg : '模块标签修改成功!',
