 ...
 * @author lianglp
 * @version 1.0
 * @class com.bhtec.view.business.uum.group.roleGroup.RoleGroupList
 * @date 2011-1-27
 */
Ext.namespace('com.bhtec.view.business.uum.group.roleGroup');
com.bhtec.view.business.uum.group.roleGroup.RoleGroupList = function(config){
	var xmlDoc = config.xmlDoc;//域描述信息
	var roleGroupName_q = 'roleGroupName_q';
	var moduleGridId = 'roleGroupGridId';
	...
	var queryCondition = function(){ 
		...
		queryArr.push({
					...
					items : [com.bhtec.view.util.CommonWidgets.prototype.textField({
								id:roleGroupName_q,
								width:150,
								fieldLabel : getFormFieldName(xmlDoc,'groupName'),
								listeners: {
	...
	var query = function(){		
		var configQuery = {
				url : 'roleGroupAction!findRoleGroupByCon.action',
				params : {
					groupName : getExtCmpValueById(roleGroupName_q)
				},
	 ...
	 */
	var reset = function(){
		resetCmpValueById(roleGroupName_q);
	}
	//grid row expander
    ...
    expander.on("expand",function(expander,record,body,rowIndex){ 
        			var grid = new Ext.grid.GridPanel({
				        store: new Ext.data.JsonStore({
				            fields: ['memberResourceCode','memberResourceName'],
				        ...
				        cm: new Ext.grid.ColumnModel({				           
				            columns: [
				                ...
				                {
				                	header: '<span style=color:blue;>'+getFormFieldName(xmlDoc,'roleName')+' </span>',  
				                ...
				                {
				                	header: '<span style=color:blue;>'+getFormFieldName(xmlDoc,'roleMemo')+' </span>',  
				                	dataIndex: 'memberResourceCode',
				                	width : 150,
				                	sortable: false,
				                	renderer:function(value){
				                		return '<span style=color:blue;>'+value+' </span>';
	...
	var cols = function(){
		...
		colsArr.push({
				header : getFormFieldName(xmlDoc,'groupName'),
				dataIndex : 'groupName',
				width : 150,
				renderer:function(value){
	...
	var queryPara = {
			...
			queryColWidth:0.2,
			currentPosition:basicConstant.UUM+'群组管理->角色组管理'
	}
