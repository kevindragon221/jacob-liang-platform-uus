 ...
 * @author lianglp
 * @version 1.0
 * @class com.bhtec.view.business.uum.group.UserGroupList
 * @date 2011-1-22
 */
Ext.namespace('com.bhtec.view.business.uum.group.userGroup');
com.bhtec.view.business.uum.group.userGroup.UserGroupList = function(config){
	var xmlDoc = config.xmlDoc;//域描述信息
	var userGroupName_q = 'userGroupName_q';
	var moduleGridId = 'userGroupGridId';
	...
	var query = function(){		
		var configQuery = {
				url : 'userGroupAction!findUserGroupByCon.action',
				params : {
					groupName : getExtCmpValueById(userGroupName_q)
				},
	 ...
	 */
	var reset = function(){
		resetCmpValueById(userGroupName_q);
	}
	//grid row expander
        			...
        			var grid = new Ext.grid.GridPanel({
				        ...
				        cm: new Ext.grid.ColumnModel({				           
				            columns: [
				                ...
				                {
				                	header: '<span style=color:blue;>'+getFormFieldName(xmlDoc,'userCode')+' </span>',  
				                	...
				                	header: '<span style=color:blue;>'+getFormFieldName(xmlDoc,'userName')+' </span>',  
	...
	var queryPara = {
			...
			queryColWidth:0.2,
			currentPosition:basicConstant.UUM+'群组管理->用户组管理'
	}
