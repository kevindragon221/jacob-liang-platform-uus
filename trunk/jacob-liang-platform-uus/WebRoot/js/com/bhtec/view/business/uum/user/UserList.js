...
com.bhtec.view.business.uum.user.UserList = function(config){
	...
	var cols = function(){
		...
		colsArr.push({
				header : getFormFieldName(xmlDoc,'userCode'),
				dataIndex : 'userCode',
				width : 60,
				sortable: true 
			});
		colsArr.push({
				header : getFormFieldName(xmlDoc,'userName'),
				dataIndex : 'userName',
				width : basicConstant.GRID_COL_WIDTH,
				sortable: true 
			});
		colsArr.push({
				header : getFormFieldName(xmlDoc,'userGender'),
		...
		colsArr.push({
				header : getFormFieldName(xmlDoc,'userPosition'),
		...
		colsArr.push({
				header : getFormFieldName(xmlDoc,'uumRole'),
		...
		colsArr.push({
				header : getFormFieldName(xmlDoc,'uumOrgan'),
		...
		colsArr.push({
				header : getFormFieldName(xmlDoc,'userQq'),
				dataIndex : 'userQq',
				renderer:function(value){
					return '<span qtip="'+value+'">'+value+'</span>';
				},
		...
		colsArr.push({
				header : getFormFieldName(xmlDoc,'userMobile'),
		...
		colsArr.push({
				header : getFormFieldName(xmlDoc,'userEmail'),
				dataIndex : 'userEmail',
				renderer:function(value){
					if(value == null)return;
					return '<span qtip="'+value+'">'+value+'</span>';
    ...
    var toolbar = function(){
		...
		for(i=0;i<modOptList.length;i++){
			...
			}else if('assignrole' == modOpt.optFunLink){
				handlerFun = userAssignRole;
			}else if('resetpwd' == modOpt.optFunLink){
				handlerFun = function(){
						var modDelRecord = modifyDelSelRecord(userGridId);//请选择一条件记录
						if(modDelRecord != ''){
							ajaxRequest({
