...
com.bhtec.view.business.uum.user.UserVOp = function(config){
	...
	var busiForm = function(configForm){
		...
		var userCode = moduleVOp.textField({
				...
				name : "userCode",
				allowBlank:false,
				fieldLabel : getFormFieldName(xmlDoc,'userCode'),
				value:moduleData.userCode||'',
				vtype:'alphanum',
		...
		var userName = moduleVOp.textField({
				...
				allowBlank:false,
				fieldLabel : getFormFieldName(xmlDoc,'userName')			
		});
		...
		var userPassword = moduleVOp.textField({
				...
				inputType:'password',
				allowBlank:false,
				fieldLabel : getFormFieldName(xmlDoc,'userPassword'),
				vtype:'alphanum',
				maxLength:10,
				emptyText:'请输入字母、数字、下划线'
		});
		...
		var userGender = moduleVOp.radio({
				...
				fieldLabel : getFormFieldName(xmlDoc,'userGender'),
				width:150,
				items:[
		...
		var userPosition = moduleVOp.textField({
				...
				fieldLabel : getFormFieldName(xmlDoc,'userPosition'),
				maxLength:20
		});
		var userPhotoUrl = (moduleData.userPhotoUrl==undefined||moduleData.userPhotoUrl=='')?'./img/business/blank_userphoto.gif':(basicConstant.ACCESSORY_IMG_PATH)+moduleData.userPhotoUrl;
		//用户照片
		var userPhotoView = moduleVOp.textField({
				...
				width:150,
				height:100,
				fieldLabel : getFormFieldName(xmlDoc,'userPhotoUrl'),
			    autoCreate : {
				     tag : 'input',
				     type : 'image',
				     src : userPhotoUrl,
			                ...
			                if(fileUrlStandard.test(fileUrl)){
						     	  if (Ext.isIE) {
								       ...
								       image.filters.item("DXImageTransform.Microsoft.AlphaImageLoader").src = fileUrl;
							       	   ...
							       	   getExtCmpById('selectPhotoId').dom.src = fileUrl;
							      }
						     }else{
						     	var obj = Ext.getDom('fileUpload');
						     	obj.select();   
		...
		var userQq = moduleVOp.numberField({
				...
				maxLength:20,
				fieldLabel : getFormFieldName(xmlDoc,'userQq'),
				allowDecimals : false,
				emptyText:'请输入数字'
		});
		//用户MSN
		var userMsn = moduleVOp.textField({
				...
				fieldLabel : getFormFieldName(xmlDoc,'userMsn'),
				vtype:'email',
				maxLength:30
		});
		...
		var userMobile = moduleVOp.numberField({
				name : "userMobile",
				fieldLabel : getFormFieldName(xmlDoc,'userMobile'),
		...
		var userMobile2 = moduleVOp.numberField({
				name : "userMobile2",
				fieldLabel : getFormFieldName(xmlDoc,'userMobile2'),
				allowDecimals : false ,
				maxLength:20,
		...
		var userOfficeTel = moduleVOp.numberField({
				name : "userOfficeTel",
				fieldLabel : getFormFieldName(xmlDoc,'userOfficeTel'),
		...
		var userFamilyTel = moduleVOp.numberField({
				name : "userFamilyTel",
				fieldLabel : getFormFieldName(xmlDoc,'userFamilyTel'),
		...
		var userAddress = moduleVOp.textField({
				name : "userAddress",
				fieldLabel : getFormFieldName(xmlDoc,'userAddress'),
		...
		var userEmail = moduleVOp.textField({
				name : "userEmail",
				fieldLabel : getFormFieldName(xmlDoc,'userEmail'),
		...
		var userAvidate = moduleVOp.dateField({
				name : "userAvidate",
				fieldLabel : getFormFieldName(xmlDoc,'userAvidate'),
				format:'Y-m-d',
		...
		var status = moduleVOp.comboBox({
				id : "status",
				fieldLabel : getFormFieldName(xmlDoc,'status'),
						...
						items	 : moduleVOp.asyncTreePanel({
										...
										clickNode:function(node, e){
											...
											if(nodeId.substr(_flag+1) == '1' || nodeId == '0'){
												...
												getExtCmpById('uumRoleNameId').setValue(node.text);
											}
										}
								})
					});
		...
		var uumRole = moduleVOp.triggerField({
				id : 'uumRoleNameId',
				name : "uumRoleName",
				value:moduleData.uumRoleName||'',
				allowBlank:false,
				fieldLabel : getFormFieldName(xmlDoc,'uumRole'),
				allowBlank:false,
				window:treeWin
		});
		...
		var memo = moduleVOp.textField({
				...
				fieldLabel : getFormFieldName(xmlDoc,'memo'),
				maxLength:50
		});
