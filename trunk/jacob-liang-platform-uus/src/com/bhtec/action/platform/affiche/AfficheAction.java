...
import static com.bhtec.common.constant.PojoVariable.SYSPL_ACCESSORY;
import static com.bhtec.common.constant.PojoVariable.SYSPL_AFFICHE;
...
public class AfficheAction extends PlatformBaseAction implements ModelDriven<SysplAffiche> {
	...
	public void saveAffiche() {
		try {
			...
			long id = this.getPrimaryKeyByPojoName(SYSPL_AFFICHE);
			sysplAffiche.setAfficheId(id);
			...
			if(accessoryUpload != null){
				...
				for(File accessoryFile:accessoryUpload){
					...
					long accessoryId = this.getPrimaryKeyByPojoName(SYSPL_ACCESSORY);
					sysplAccessory.setAccessoryId(accessoryId);
					sysplAccessory.setInfoId(id);
