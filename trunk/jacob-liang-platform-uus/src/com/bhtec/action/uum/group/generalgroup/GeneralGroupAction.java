 ...
 */
package com.bhtec.action.uum.group.generalgroup;
...
public class GeneralGroupAction extends PlatformBaseAction implements ModelDriven<UumGroup> {
	private static final long serialVersionUID = 10000L;
	Logger log = Logger.getLogger(this.getClass());
	private UumGroup uumGroup = new UumGroup();
	private String failMesg;
	private List<String> uumGroupListStr;
	...
	public void saveUumGroup() {
		try {
			generalGroupService.setHttpServletRequest(this.getHttpServletRequest());
			long id = this.getPrimaryKeyByPojoName(UUM_GROUP);
			uumGroup.setGroupId(id);
			uumGroup.setGroupType(GENERAL_GROUP);
			...
			if(uumGroupListStr != null && !uumGroupListStr.isEmpty()){
				for(int i=0,j=uumGroupListStr.size();i<j;i++){
					String memberResourceId_type = uumGroupListStr.get(i);
					if(!isNullOrEmpty(memberResourceId_type)){
						int flag = memberResourceId_type.indexOf("_");
						long memberResourceId = Long.parseLong(memberResourceId_type.substring(0,flag));
						String memberType = memberResourceId_type.substring(flag+1);
						...
						uumGroupMember.setGroupMemberType(memberType);
			...
			generalGroupService.saveUumGroup(uumGroup);
			this.returnSuccess();
	...
	public void modifyUumGroup(){
		try {
			...
			uumGroup.setUumGroupMembers(uumGroupMemberSet);
			uumGroup.setGroupType(GENERAL_GROUP);
			generalGroupService.modifyUumGroup(uumGroup);
			this.returnSuccess();
		} catch (ApplicationException e) {
			e.printStackTrace();
			this.returnFailur(e.toString());
	 ...
	 */
	public String findGeneralGroupByCon(){
		...
		Map map = generalGroupService.findGeneralGroupByCon(getStart(),getLimit(),
				uumGroup.getGroupName(),uumUserList,uumRoleList);
	...
	public String findGroupNameIsExist(){
		groupNameIsExist = generalGroupService.findGroupNameIsExist(uumGroup.getGroupName());
		return this.SUCCESS;
	}
	...
	public String disEnableGroup()throws ApplicationException{
		try {
			generalGroupService.setHttpServletRequest(this.getHttpServletRequest());
			disEnableBol = generalGroupService.disEnableGroup(new Long(getModViewRecId()), disEnableFlag);
	...
	public String findOrganRoleRefByOrgRolId(){
		try {
			userList = generalGroupService.findGeneralByOrgRolId(organId, roleId, uumGroup.getGroupType());
		} catch (ApplicationException e) {
			e.printStackTrace();
			failMesg = e.toString();
		}catch (SystemException e) {
	...
	public void setGeneralGroupService(GeneralGroupService generalGroupService) {
	...
	public List<String> getUumGroupListStr() {
		return uumGroupListStr;
	}

	public void setUumGroupListStr(List<String> uumGroupListStr) {
