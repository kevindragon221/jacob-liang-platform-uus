...
public class UserAction extends UumBaseAction  implements ModelDriven<UumUser> {
	...
	public void saveUser(){
		try {
			...
			long id = this.getPrimaryKeyByPojoName(UUM_USER);
			uumUser.setUserId(id);
	...
	public String userAssignOptPrivilege(){
		Map<String,List<SysplModuleMemu>> moduleMemuMap = 
			privilegeService.findSeledAUnseledModOptByOwnIdAPriType(uumUser.getUserId(), USER_PRIVILEGE);
	...
	public String saveUserOptPrivilege(){
		try {
			...
			privilegeService.savePrivilege(modOptIdList, uumUser.getUserId(), USER_PRIVILEGE);
		} catch (ApplicationException e) {
			e.printStackTrace();
			failMesg = e.toString();
		}catch (SystemException e) {
