...
import static com.bhtec.common.constant.PojoVariable.SYSPL_MODULEOPT_REF;
...
import static com.bhtec.common.constant.ServiceVariable.MAINFRAME_FILE_NAME;
...
public class ModuleServiceImpl extends BaseServiceImpl implements ModuleService {
	...
	public boolean disEnableModule(long moduleId,String disEnableFlag)throws ApplicationException{
		if(DISABLE.equals(disEnableFlag)){
			...
			if(modCount > 0){//有下级菜单
				...
				this.modifyModule(sysplModuleMemu);
	...
	public void saveModuleOptRefs(long moduleId, List<Long> operateIdList)throws ApplicationException {
		...
		if(sysplModOptRefList == null || sysplModOptRefList.size()==0){//如果没有任何操作则直接增加
			...
			this.moduleDao.deleteAll(sysplModOptRefList);//进行模块操作关系删除并级联删除权限操作
	...
	private void saveModuleOptRefSingle(long moduleId, List<Long> operateIdList)throws ApplicationException{
		...
		if(operateIdList != null && operateIdList.size()>0){
			for(Long operateId:operateIdList){
				...
				Long modOptId = this.getPrimaryKeyByPojoName(SYSPL_MODULEOPT_REF);
				...
				this.moduleDao.save(sysplModOptRef);
	 ...
	 * 功能说明：修改行权限
	 * @author jacobliang
	 * @param moduleIdList 模块集合
	 * @param rowPrivilegeLevel	权限类型
	 * @time Nov 7, 2010 2:34:01 PM
	 */
	public void modifyModuleRowPrivilege(List<Long> moduleIdList,String rowPrivilegeLevel) throws ApplicationException{
		String moduleIds = "";
		if(moduleIdList != null && moduleIdList.size()>0){
			...
			this.moduleDao.modifyModuleRowPrivilege(moduleIds, rowPrivilegeLevel);
			StringBuffer logContent = new StringBuffer();
			logContent.append("修改行权限为：").append(rowPrivilegeLevel);
			saveLog(LOG_LEVEL_SECOND, SYS_MOD_MGR,MODIFY_OPT,logContent.toString(), moduleIdList.toString());
		}
	}
