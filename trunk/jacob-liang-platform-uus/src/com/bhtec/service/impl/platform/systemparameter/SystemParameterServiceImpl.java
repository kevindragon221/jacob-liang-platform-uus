...
public class SystemParameterServiceImpl extends BaseServiceImpl implements SystemParameterService {
	...
	public Map findSystemParaByCon(int start,int limit,String paraName,String paraKeyName){
		List<SysplDicSmallType> sysplDicSmallTypeList = typeDictionaryService.findSmallTypeDicByBigTypeCode(SYSTEM_TYPE);
		Map map =  this.systemParameterDao.findSystemParaByCon(start, limit, paraName, paraKeyName);
		List<SysplSysParameter> limitModuleList = (List<SysplSysParameter>)map.get(BUSI_LIST);
		for(SysplSysParameter sysplSysParameter:limitModuleList){
			String paraType = sysplSysParameter.getParaType();
			for(SysplDicSmallType sysplDicSmallType:sysplDicSmallTypeList){
				if(paraType.equalsIgnoreCase(sysplDicSmallType.getSmallTypeCode())){
					sysplSysParameter.setParaType(sysplDicSmallType.getSmallTypeName());
					break;
				}
			}
		}
