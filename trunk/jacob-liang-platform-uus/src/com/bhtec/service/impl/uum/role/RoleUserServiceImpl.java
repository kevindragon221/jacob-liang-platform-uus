...
public class RoleUserServiceImpl extends BaseServiceImpl implements RoleUserService {
	 ...
	 */
	public void saveRoleUserRef(long userId, List<Long> orgRoleId,long dfaultOrgRoleId,long orgRoleUserId) throws ApplicationException {
		String orgRoleIdStr = "";
		for(Long orgRoleid:orgRoleId){
			UumRoleUserRef uumRoleUserRef = new UumRoleUserRef();
			
			long orgRoleUserid = orgRoleUserId==0?this.getPrimaryKeyByPojoName(UUM_ROLE_USER_REF):orgRoleUserId;
			uumRoleUserRef.setOrgRoleUserId(orgRoleUserid);
			...
			this.getBaseDao().save(uumRoleUserRef);
	...
	public void saveRoleUser(long userId,String orgRoleIds,long defaultOrgRoleId)throws ApplicationException{
		...
		//为用户添加角色
		this.saveRoleUserRef(userId, orgRoleIdList, defaultOrgRoleId, 0);
	}
