...
import static com.bhtec.common.constant.PojoVariable.UUM_ROLE_USER_REF;
...
public class UserServiceImpl extends BaseServiceImpl implements UserService {
	...
	public boolean disEnableUser(long userId, String disEnableFlag)
		...
		if(DISABLE.equals(disEnableFlag)){
			...
			long orgRoleUserid = this.getPrimaryKeyByPojoName(UUM_ROLE_USER_REF);
			List<Long> orgRole = new ArrayList<Long>();
			orgRole.add(0L);
			roleUserService.setHttpServletRequest(this.getRequest());
			roleUserService.saveRoleUserRef(userId, orgRole, 0, orgRoleUserid);//移到无角色用户
			privilegeService.deletePrivilege(userId, USER_PRIVILEGE);//删除用户所拥有操作权限
			UumOrgan uumOrgan = new UumOrgan();
			uumOrgan.setOrgId(0L);
			uumUser.setUumOrgan(uumOrgan);//更改用户所属机构为0
			this.modifyUser(uumUser);
	...
	public void saveUser(UumUser uumUser) throws ApplicationException {
		...
		list.add(uumUser.getUumOrgRoleId());
		long orgRoleUserid = this.getPrimaryKeyByPojoName(UUM_ROLE_USER_REF);
		roleUserService.setHttpServletRequest(this.getRequest());
		roleUserService.saveRoleUserRef(uumUser.getUserId(),list, uumUser.getUumOrgRoleId(),orgRoleUserid);//保存角色用户
